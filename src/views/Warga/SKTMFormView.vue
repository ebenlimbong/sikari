...
const handleSubmit = async () => {
  ...
  try {
    // Simulasi API call
    console.log('📝 Data Pengajuan SKTM:', formData.value);
    
    // Simulasi upload files
    const formDataToSend = new FormData();
    Object.keys(formData.value).forEach(key => {
      if (key === 'files') {
        Object.keys(formData.value.files).forEach(fileKey => {
          if (formData.value.files[fileKey]) {
            formDataToSend.append(fileKey, formData.value.files[fileKey]);
          }
        });
      } else {
